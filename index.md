...
`OpenEphys.Onix1` is a [Bonsai](https://bonsai-rx.org/) library that can be used to control the [ONIX PCIe Acquisition System](https://open-ephys.org/onix/oeps-9006). 
...
* Automatic synchronization of all data streams.
* Torque-free commutation of ultra-thin (down to ~0.2mm diameter) tethers
* High performance closed-loop control (100 usec loop times)
...
ONIX hardware, and the [ONI specification and
API](https://open-ephys.github.io/ONI/) that is built on, are software agnostic.
Bonsai is the first software target pursued by the Open Ephys team for ONIX
hardware. There are three major reasons for this:
...
can
   ...
   processing data from essentially any data source regardless of their  sample
   ...
   [mumpy.ndarray](https://numpy.org/doc/stable/reference/generated/numpy.ndarray.html#numpy.ndarray).
   And, just like Numpy offers an extensive toolkit of linear algebra functions
   to operate on these arrays, Bonsai offers an analogous
   [toolkit](https://reactivex.io/documentation/operators.html) that defines an
   algebra for operating on temporal sequences. Because Bonsai was created around this core
   ...
   bug prone and manual in other software options.
...
hundreds of pieces of open- and closed-source hardware and software that are used extensively in
neuroscience research. From instance:
...
> [!NOTE]
> We realize that Bonsai can be difficult to learn. We put a lot of effort into
> making these docs useful for all users. If you have suggestions for making
